...
{def $err=false
     ...
     $invalid_length=ne( count( $data_text ), 10 )
}
...
<div id="{$attribute.id}optionsection" class="{if and(ne($supress_validation, true), eq( $err, true))}option_error{else}option_label{/if}"><label id="{$attribute.id}optionlabel" class="{if and(ne($supress_validation, true), eq( $err, true))}option_text_error{/if}">{if $attribute.is_required}<span class="field_error_message">*&nbsp;</span>{/if}{$attribute.contentclass_attribute_name}</label><input onchange="FilterNumericInput(this)" class="{eq( $html_class, 'half' )|choose( 'box', 'halfbox' )}{if eq($attribute.is_required, 1)} validate_phone{/if}" type="text" size="70" name="{$attribute_base}_bcphonestring_data_text_{$attribute.id}" value="{$data_text|wash( xhtml )}" />

<div class="form_field_error_message" id="{$attribute_base}_bcphonestring_data_text_{$attribute.id}_error" {if or(eq( $supress_validation, true ),ne( $err, true))} style="display:none;"{/if}>Please provide a {$attribute.contentclass_attribute_name|downcase()}{* contact number *} with at least 10 digits</div>
