<?php
//
// Definition of BcPhoneStringType class
//
...
/*!
  \class BcPhoneStringType bcphonestringtype.php
  \ingroup eZDatatype
...
define( 'EZ_DATATYPEPHONESTRING_STRING', 'bcphonestring' );
define( 'EZ_DATATYPEPHONESTRING_MAX_LEN_FIELD', 'data_int1' );
define( 'EZ_DATATYPEPHONESTRING_MAX_LEN_VARIABLE', '_bcphonestring_max_string_length_' );
define( "EZ_DATATYPEPHONESTRING_DEFAULT_STRING_FIELD", "data_text1" );
define( "EZ_DATATYPEPHONESTRING_DEFAULT_STRING_VARIABLE", "_bcphonestring_default_value_" );

class BcPhoneStringType extends eZDataType
{
    /*!
     Initializes with a string id and a description.
    */
    function BcPhoneStringType()
    {
        $this->eZDataType( EZ_DATATYPEPHONESTRING_STRING, ezi18n( 'kernel/classes/datatypes', 'Phone Number', 'Datatype name' ),
...
//                                                                         $currentVersion );
    ...
    */
    ...
    {
        $maxLen = $classAttribute->attribute( EZ_DATATYPEPHONESTRING_MAX_LEN_FIELD );
	...
	elseif ( !is_numeric( $textCodec->strlen( $data ) ) ) {
	  $contentObjectAttribute->setValidationError( ezi18n( 'kernel/classes/datatypes',
                                                               'The input text must contain only numbers. The maximum number of characters allowed is %1.' ),
						       $maxLen );
	  return EZ_INPUT_VALIDATOR_STATE_INVALID;
	}
    ...
    */
    ...
    {
        if ( $http->hasPostVariable( $base . '_bcphonestring_data_text_' . $contentObjectAttribute->attribute( 'id' ) ) )
        {
            $data = $http->postVariable( $base . '_bcphonestring_data_text_' . $contentObjectAttribute->attribute( 'id' ) );
    ...
    */
    function fetchCollectionAttributeHTTPInput( &$collection, &$collectionAttribute, &$http, $base, &$contentObjectAttribute )
    {
        if ( $http->hasPostVariable( $base . "_bcphonestring_data_text_" . $contentObjectAttribute->attribute( "id" ) ) )
        {
            $dataText = $http->postVariable( $base . "_bcphonestring_data_text_" . $contentObjectAttribute->attribute( "id" ) );
    ...
    */
    ...
    {
        $maxLenName = $base . EZ_DATATYPEPHONESTRING_MAX_LEN_VARIABLE . $classAttribute->attribute( 'id' );
    ...
    */
    ...
    {
        $maxLenName = $base . EZ_DATATYPEPHONESTRING_MAX_LEN_VARIABLE . $classAttribute->attribute( 'id' );
        $defaultValueName = $base . EZ_DATATYPEPHONESTRING_DEFAULT_STRING_VARIABLE . $classAttribute->attribute( 'id' );
        if ( $http->hasPostVariable( $maxLenName ) )
        {
            $maxLenValue = $http->postVariable( $maxLenName );
            $classAttribute->setAttribute( EZ_DATATYPEPHONESTRING_MAX_LEN_FIELD, $maxLenValue );
        }
        if ( $http->hasPostVariable( $defaultValueName ) )
        {
            $defaultValueValue = $http->postVariable( $defaultValueName );

            $classAttribute->setAttribute( EZ_DATATYPEPHONESTRING_DEFAULT_STRING_FIELD, $defaultValueValue );
        }
        return true;
    }
    ...
    */
    ...
    {
        $maxLength = $classAttribute->attribute( EZ_DATATYPEPHONESTRING_MAX_LEN_FIELD );
        $defaultString = $classAttribute->attribute( EZ_DATATYPEPHONESTRING_DEFAULT_STRING_FIELD );
    ...
    */
    function unserializeContentClassAttribute( &$classAttribute, &$attributeNode, &$attributeParametersNode )
    {
        ...
        $classAttribute->setAttribute( EZ_DATATYPEPHONESTRING_MAX_LEN_FIELD, $maxLength );
        $classAttribute->setAttribute( EZ_DATATYPEPHONESTRING_DEFAULT_STRING_FIELD, $defaultString );
    }
...
eZDataType::register( EZ_DATATYPEPHONESTRING_STRING, 'bcphonestringtype' );
